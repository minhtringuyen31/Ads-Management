...
import { getAllReportByLocation } from 'lib/api';
...
const MapBox = () => {
  ...
  const {
    ...
    error,
  } = useHttp(getAllReportByLocation, true);

  const handleOpenLocationDetailClick = (list) => {
    reportMapCtx.setReportsDetail(list);
  };
  ...
  if (status === 'completed') {
    ...
    if (loadedLocations.length > 0) {
      ...
      loadedLocations.forEach((location, index) => {
              ...
              >
                ...
                {loadedLocations.map((location) => (
                  <Marker
                    ...
                    eventHandlers={{
                      click: () => {
                        console.log(location.reportList);
                        handleOpenLocationDetailClick(location.reportList);
                      },
                    }}
                  />
                ))}

...
const DeleteModal = ({
  ...
  const handleDeleteSubmit = async () => {
    try {
      if (type === 'district') {
        await instance.delete(`${rootApi}/district/${id}`);
        handleDeleteClose();
        handleTriggleList();
      } else {
        await instance.delete(`${rootApi}/ward/${id}`);
        handleDeleteClose();
        handleTriggleList();
      }
    } catch (error) {
        ...
        <Grid container spacing={3}>
          <Grid item xs={12}>
            <Typography id='modal-modal-title' variant='h3' component='h2'>
              {type === 'district'
                ? 'Bạn có chắc chắn muốn xóa Quận/Huyện này?'
                : 'Bạn có chắc chắn muốn xóa Phường/Xã này?'}
            </Typography>
          </Grid>
          <Grid item xs={12}>

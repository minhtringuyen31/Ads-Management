...
const UserService = {
  async getAll(filter, projection) {
    try {
      const wardOfficers = await WardOfficer.find({ __t: "WardOfficer" })
        .populate({
      ...
      const provinceOfficers = await ProvinceOfficer.find({
        __t: "ProvinceOfficer",
      });
  ...
  async getById(id) {
    try {
      ...
      if (user.__t === "DistrictOfficer") {
        user = await DistrictOfficer.findById(id)
          .populate({
            path: "assigned_areaid",
        ...
        user = await WardOfficer.findById(id)
          .populate({
            path: "assigned_areaid",
        ...
        user = await ProvinceOfficer.findById(id);
      }
      console.log(user);
  ...
  async update(id, data) {
    try {
      ...
      let updatedUser;
      if (user.__t === "DistrictOfficer") {
        updatedUser = await DistrictOfficer.findOneAndUpdate(
          { _id: id },
          { $set: data },
          { new: true }
        );
      } else if (user.__t === "WardOfficer") {
        updatedUser = await WardOfficer.findOneAndUpdate(
          { _id: id },
          { $set: data },
          { new: true }
        );
